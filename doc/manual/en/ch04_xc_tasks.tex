...
\item [\bmenuw{Save}] Saves the current active task.  Upon tapping the 
  ...
  task to be saved.
\item [\bmenuw{Download declared task from WeGlide}] Downloads the currently declared task from WeGlide.
  Upon tapping the \bmenuw{Download declared task from WeGlide} the file is saved to a weglide subfolder
  inside the XCSoarData folder.
  To activate the task, it has to be selected with the \bmenuw{Browse} button.
  This option is only available, if the Weglide Pilod-Id is configured under
  Settings -> Settings -> WeGlide.
\end{itemize}
